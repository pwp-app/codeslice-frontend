    ...
    <div class="container-present">
        <div class="present">
            <transition>
                ...
                <div class="container present-main" v-if="slice.content && !slice.loading">
                    <Header></Header>
                    ...
                    <div class="present-bottom">
                        ...
                        <div class="present-action">
                            <el-button type="primary" @click="doCopy">复制到剪贴板</el-button>
                            <el-button type="primary" @click="doSave">下载为文本</el-button>
                        </div>
...
export default {
        ...
        this.axios
            ...
            .then(res => {
                if (res && res.data.code === 200) {
                    if (res.data.data.createTime + res.data.data.expires <= new Date().getTime() / 1000) {
                        this.slice.loading = false;
                        return;
                    }
                    this.slice = res.data.data;
                    this.slice.createTime = dayjs(this.slice.createTime).format('YYYY-MM-DD HH:mm:ss');
                    this.slice.expires = dayjs(this.slice.createTime).add(this.slice.expires, 's').format('YYYY-MM-DD HH:mm:ss');
    ...
    methods: {
        doCopy() {
            this.$copyText(this.slice.content).then(e => {
                this.$message({
                    type: 'success',
                    message: '代码已复制到剪贴板'
                });
